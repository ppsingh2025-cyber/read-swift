...
|--------|------|--------|--------|
| `parsePDF` | `parsers/pdfParser.ts` | PDF | page text (pdfjs-dist, `hasEOL` for line breaks). Token-level diagram filtering: items whose trimmed text is ≤2 chars of letter/arrow characters are classified as diagram labels and collapsed into a `\n[Figure]\n` placeholder. Prose tokens always pass through unaffected. |
